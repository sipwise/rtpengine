...
static int output_flush(output_t *output) {
	while (av_audio_fifo_size(output->fifo) >= output->frame->nb_samples) {
		...
		dbg("%p output fifo pts %lu", output, output->fifo_pts);
		...
		dbg("%p output pts/dts is %li/%li", output, output->avpkt.pts, output->avpkt.dts);
...
static int output_add(output_t *output, AVFrame *frame) {
	...
	dbg("%p output fifo size %u fifo_pts %lu", output, av_audio_fifo_size(output->fifo),
			output->fifo_pts);
	// fix up output pts
...
int decoder_input(decoder_t *dec, const str *data, unsigned long ts, output_t *output) {
	...
	dbg("%p dec pts %lu rtp_ts %lu incoming ts %lu", dec, dec->pts, dec->rtp_ts, ts);
	...
	dbg("%p dec frame pts %lu pkt_pts %lu", dec, dec->frame->pts, dec->frame->pkt_dts);

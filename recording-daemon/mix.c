...
format_mismatch:
	...
	for (int i = 0; i < G_N_ELEMENTS(mix->src_ctxs); i++) {
		...
		av_opt_set_int(mix->src_ctxs[i], "channel_layout",
				av_get_default_channel_layout(mix->channels), AV_OPT_SEARCH_CHILDREN);
		av_opt_set_int(mix->src_ctxs[i], "sample_fmt",
				AV_SAMPLE_FMT_S16, AV_OPT_SEARCH_CHILDREN);
		av_opt_set_int(mix->src_ctxs[i], "sample_rate",
				mix->clockrate, AV_OPT_SEARCH_CHILDREN);
		av_opt_set_q(mix->src_ctxs[i], "time_base",
...
err:
	mix_destroy(mix);
	ilog(LOG_ERR, "Failed to create mixer: %s", err);
	return -1;
}

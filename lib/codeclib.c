...
static void dtmf_decoder_close(decoder_t *);
...
//		enc->fifo_pts = frame->pts;
	...
	ilog(LOG_DEBUG, "xxx fifo pts %lu", enc->fifo_pts);
...
static int dtmf_decoder_input(decoder_t *dec, const str *data, GQueue *out) {
	...
	// init if we need to
	if (dtmf->event != dec->u.dtmf.event) {
		ZERO(dec->u.dtmf);
	...
	// XXX adjust parallel dec pts

	return 0;
}

static void dtmf_decoder_close(decoder_t *dec) {
}

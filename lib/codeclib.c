...
#define CODEC_DEF_FULL(ref, codec_id, mult, name, clockrate, channels, bitrate, ptime, pizer) { \
	.rtpname = #ref, \
	.avcodec_id = codec_id, \
	.clockrate_mult = mult, \
	.avcodec_name = #name, \
...
#define CODEC_DEF_AVC(ref, id, mult, name, clockrate, channels, bitrate, ptime, pizer) \
	CODEC_DEF_FULL(ref, AV_CODEC_ID_ ## id, mult, name, clockrate, channels, bitrate, ptime, pizer)
#define CODEC_DEF_MULT_NAME(ref, id, mult, name, pizer) CODEC_DEF_AVC(ref, id, mult, name, -1, -1, 0, 0, pizer)
#define CODEC_DEF_MULT(ref, id, mult, pizer) CODEC_DEF_MULT_NAME(ref, id, mult, NULL, pizer)
#define CODEC_DEF_NAME(ref, id, name, pizer) CODEC_DEF_MULT_NAME(ref, id, 1, name, pizer)
#define CODEC_DEF(ref, id, pizer) CODEC_DEF_MULT(ref, id, 1, pizer)

// _ENC macros provided for codecs not having defaults in the RTP RFC
#define CODEC_DEF_NAME_ENC(ref, id, name, clockrate, channels, bitrate, ptime, pizer) \
	CODEC_DEF_MULT_NAME_ENC(ref, id, 1, name, clockrate, channels, bitrate, ptime, pizer)
#define CODEC_DEF_MULT_ENC(ref, id, mult, clockrate, channels, bitrate, ptime, pizer) \
	CODEC_DEF_MULT_NAME_ENC(ref, id, mult, NULL, clockrate, channels, bitrate, ptime, pizer)
#define CODEC_DEF_ENC(ref, id, clockrate, channels, bitrate, ptime, pizer) \
	CODEC_DEF_MULT_ENC(ref, id, 1, clockrate, channels, bitrate, ptime, pizer)
#define CODEC_DEF_MULT_NAME_ENC(ref, id, mult, name, clockrate, channels, bitrate, ptime, pizer) \
	CODEC_DEF_AVC(ref, id, mult, name, clockrate, channels, bitrate, ptime, pizer)

// not real audio codecs
#define CODEC_DEF_STUB(ref) CODEC_DEF_FULL(ref, -1, 1, ref, -1, -1, 0, 0, passthrough)

static const struct codec_def_s codecs[] = {
	CODEC_DEF(PCMA, PCM_ALAW, samplestream),
	CODEC_DEF(PCMU, PCM_MULAW, samplestream),
	CODEC_DEF(G723, G723_1, passthrough),
	CODEC_DEF_MULT(G722, ADPCM_G722, 2, samplestream),
	CODEC_DEF(QCELP, QCELP, passthrough),
	CODEC_DEF(G729, G729, passthrough),
	CODEC_DEF_ENC(speex, SPEEX, 16000, 1, 11000, 20, passthrough),
	CODEC_DEF(GSM, GSM, passthrough),
	CODEC_DEF(iLBC, ILBC, passthrough),
	CODEC_DEF_NAME_ENC(opus, OPUS, libopus, 48000, 2, 32000, 20, passthrough),
	CODEC_DEF_NAME(vorbis, VORBIS, libvorbis, passthrough),
	CODEC_DEF(ac3, AC3, passthrough),
	CODEC_DEF(eac3, EAC3, passthrough),
	CODEC_DEF(ATRAC3, ATRAC3, passthrough),
	CODEC_DEF(ATRAC-X, ATRAC3P, passthrough),
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(57, 0, 0)
	CODEC_DEF(EVRC, EVRC, passthrough),
	CODEC_DEF(EVRC0, EVRC, passthrough),
	CODEC_DEF(EVRC1, EVRC, passthrough),
#endif
	CODEC_DEF_ENC(AMR, AMR_NB, 8000, 1, 6600, 20, passthrough),
	CODEC_DEF_ENC(AMR-WB, AMR_WB, 16000, 1, 14250, 20, passthrough),
...
int packetizer_passthrough(AVPacket *pkt, GString *buf, str *output) {
	str_init_len(output, (char *) pkt->data, pkt->size);
	return 0;
}

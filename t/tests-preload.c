...
int bind(int fd, const struct sockaddr *addr, socklen_t addrlen) {
	...
	switch (s->wanted_domain) {
		case AF_INET:;
			...
			if (!inet_ntop(s->wanted_domain, &sin->sin_addr, sockname, sizeof(sockname)))
				goto do_bind_warn;
		...
		case AF_INET6:;
			...
			if (addrlen > sizeof(sst))
				goto do_bind_warn;
			err = "failed to print network address";
			if (!inet_ntop(s->wanted_domain, &sin6->sin6_addr, sockname, sizeof(sockname)))
				goto do_bind_warn;
...
ssize_t sendmsg(int fd, const struct msghdr *msg, int flags) {
	...
	if (msg2.msg_name)
		msg2.msg_name = (void *) addr_find(msg2.msg_name, &msg2.msg_namelen);

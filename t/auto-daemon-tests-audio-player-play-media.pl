...
autotest_start(qw(--config-file=none -t -1 -i 203.0.113.1 -i 2001:db8:4321::1
			...
			--audio-player=play-media))
		or die;


my ($sock_a, $sock_b, $port_a, $port_b, $ssrc_a, $ssrc_b, $seq_a, $seq_b, $ts_a, $ts_b, $resp);
...
rcv_no($sock_b);
snd($sock_a, $port_b, rtp(0, 3000, 5000, 0x1234, "\x00" x 160));
rcv($sock_b, $port_a, rtpm(0, 3000, 5000, 0x1234, "\x00" x 160));
rcv_no($sock_b);
...
($sock_a, $sock_b) = new_call([qw(198.51.100.14 6020)], [qw(198.51.100.14 6022)]);

($port_a) = offer('play media',
	{ }, <<SDP);
v=0
o=- 1545997027 1 IN IP4 198.51.100.1
s=tester

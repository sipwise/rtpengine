...
struct params {
	int code, level, delay;
};

static void report_func(void *ptr, int code, int level, int delay) {
	struct params *dtmf_params = ptr;
	printf("code %i level %i delay %i\n", code, level, delay);
	dtmf_params->code = code;
	dtmf_params->level = level;
	dtmf_params->delay = delay;
}
...
int main(int argc, char **argv) {
	...
	struct params dtmf_params = {0,};

	dtmf_rx_state_t *dtmf_dsp;
	dtmf_dsp = dtmf_rx_init(NULL, NULL, NULL);
	dtmf_rx_set_realtime_callback(dtmf_dsp, report_func, &dtmf_params);

	unsigned char *reader = samples;
	unsigned char *end = samples + sizeof(samples);
	int packetise = 160;

	int expects[] = { 0, 0, 0, 56, 56, 56, 56, 56, 56, 0 };
	int num_expects = sizeof(expects) / sizeof(*expects);
	...
	while (1) {
		...
		printf("status %i %c\n", digit, digit);
		if (digit != expects[iter])
			abort();
		if (digit) {
			if (dtmf_params.code != digit)
				abort();
			if (dtmf_params.level != -4)
				abort();
		}

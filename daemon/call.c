		...
		const struct sdp_ng_flags *flags)
{
	...
	for (media_iter = streams->head; media_iter; media_iter = media_iter->next) {
		...
		if (flags->ptime > 0) {
			media->ptime_override = flags->ptime;
			if (!other_media->ptime_override)
				other_media->ptime_override = flags->ptime;
		}
		if (media->ptime_override)
			media->ptime = media->ptime_override;
		if (other_media->ptime_override)
			other_media->ptime = other_media->ptime_override;

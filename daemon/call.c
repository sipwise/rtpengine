...
#define MLL_COMMON /* common args */						\
	...
	for (l = k; l; l = l->next) {
		...
		ilog(LOG_INFO, "------ Average MOS %.1f, lowest MOS %.1f (at %u:%02u), "
				"highest MOS %.1f (at %u:%02u)",
			(double) se->mos_sum / se->stats_blocks.length / 10.0,
			(double) se->lowest_mos->mos / 10.0,
			(unsigned int) (timeval_diff(&se->lowest_mos->reported, &c->created) / 1000000) / 60,
			(unsigned int) (timeval_diff(&se->lowest_mos->reported, &c->created) / 1000000) % 60,
			(double) se->highest_mos->mos / 10.0,

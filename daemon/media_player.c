...
// call is locked in R
static void send_timer_send_rtcp(struct ssrc_ctx *ssrc_out, struct packet_stream *ps) {
...
static void send_timer_rtcp(struct send_timer *st, struct ssrc_ctx *ssrc_out) {
	...
	ilog(LOG_DEBUG, "Generating and sending RTCP report");

	send_timer_send_rtcp(ssrc_out, ps);

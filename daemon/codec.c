...
static int __rtp_payload_type_add_recv(struct call_media *media, struct rtp_payload_type *pt) {
	if (g_hash_table_lookup(media->codecs, &pt->payload_type)) {
...
static void __rtp_payload_type_add_send(struct call_media *other_media, struct rtp_payload_type *pt) {
	if (g_hash_table_lookup(other_media->codecs, &pt->payload_type)) {
		// collision/duplicate - ignore
		return;
	}
...
{
	if (__rtp_payload_type_add_recv(media, pt))
		return;

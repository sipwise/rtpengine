...
static int packet_encoded_t38(encoder_t *enc, void *u1, void *u2) {
	...
	t38_samples(mp->media->t38_encoder, mp, (int16_t *) enc->avpkt.data, enc->avpkt.size / 2);

	return 0;
}

static void __check_t38_encoder(struct call_media *receiver, struct call_media *sink) {
	if (!receiver->t38_encoder)
		receiver->t38_encoder = t38_encoder_new(receiver);
	...
	for (GList *l = receiver->codecs_prefs_recv.head; l; l = l->next) {
		...
		__make_transcoder(handler, &receiver->t38_encoder->dest_pt, NULL, -1, 0);

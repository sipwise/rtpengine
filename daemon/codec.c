...
static struct ssrc_entry *__ssrc_handler_transcode_new(void *p) {
	...
	if (h->dtmf_dsp) {
		ch->dsp_format = (format_t) { .clockrate = 8000, .channels = 1, .format = AV_SAMPLE_FMT_S16 };
...
static void __free_ssrc_handler(void *chp) {
	...
	resample_shutdown(&ch->dsp_resampler);
}
...
static int __packet_decoded(decoder_t *decoder, AVFrame *frame, void *u1, void *u2) {
	...
	if (ch->dtmf_dsp) {
		AVFrame *dsp_frame = resample_frame(&ch->dsp_resampler, frame, &ch->dsp_format);
		if (!dsp_frame)
			ilog(LOG_ERR | LOG_FLAG_LIMIT, "Failed to resample audio for DTMF DSP");
		else {
			int ret = dtmf_rx(ch->dtmf_dsp, (void *) dsp_frame->extended_data[0], dsp_frame->nb_samples);
			if (ret)
				ilog(LOG_ERR | LOG_FLAG_LIMIT, "DTMF DSP returned error");

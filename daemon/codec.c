...
void codec_handlers_update(struct call_media *receiver, struct call_media *sink) {
	...
	receiver->codec_handler_cache = NULL;
	...
	for (GList *l = sink->codecs_prefs.head; l; l = l->next) {
	...
	for (GList *l = receiver->codecs_prefs.head; l; l = l->next) {
		struct rtp_payload_type *pt = l->data;
		...
		ilog(LOG_DEBUG, "Sink does not codec " STR_FORMAT, STR_FMT(&pt->encoding));
		__make_stub(handler);
	}
}

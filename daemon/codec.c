		...
		int marker, int seq, int seq_inc)
{
	...
	// we can do output scheduling only for dynamically allocated packets
	if (p->free_func) {
		// determine scheduled time to send
		if (ch->first_send.tv_sec) {
			// scale first_send from first_send_ts to ts
			p->to_send = ch->first_send;
			unsigned long ts_diff = ts - ch->first_send_ts;
			unsigned long long ts_diff_us = ts_diff * 1000000 / ch->encoder_format.clockrate;
		...
		ilog(LOG_DEBUG, "Scheduling to send RTP packet at %lu.%06lu", p->to_send.tv_sec,
				p->to_send.tv_usec);
	}

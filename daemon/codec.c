...
static struct ssrc_entry *__ssrc_handler_new(u_int32_t ssrc, void *p) {
	...
	ilog(LOG_DEBUG, "Creating SSRC handler for SSRC %x to transcode from %s/%u/%i to SSRC %x %s/%u/%i",
			ntohl(ssrc), h->source_pt.codec_def->rtpname, h->source_pt.clock_rate,
	...
	if (!ch->encoder)
		goto err;
	if (encoder_config(ch->encoder, h->dest_pt.codec_def->avcodec_id, 0, &out_format, &ch->output_format))
		goto err;

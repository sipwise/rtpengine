...
const char *statistics_ng(bencode_item_t *input, bencode_item_t *output) {
	...
	for (GList *l = metrics->head; l; l = l->next) {
		...
		if (m->value_short) {
			if (m->is_int)
				bencode_dictionary_add_integer(dict, m->label, m->int_value);
			else
				bencode_dictionary_add_string(dict, m->label, m->value_short);
			continue;
		}
		...
		if (m->is_brace)
			sub = bencode_dictionary(dict->buffer);
		else
			sub = bencode_list(dict->buffer);
		...
		if (dict->type == BENCODE_DICTIONARY) {
			assert(sub_label != NULL);
			bencode_dictionary_add(dict, sub_label, sub);
		}

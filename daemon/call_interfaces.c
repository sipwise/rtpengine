...
INLINE void ng_sdes_option(struct sdp_ng_flags *out, str *s, void *dummy) {
	switch (__lookup(s)) {
		case STR_LOOKUP("no"):
		case STR_LOOKUP("off"):
		case STR_LOOKUP("disabled"):
		case STR_LOOKUP("disable"):
			out->sdes_off = 1;
			break;
		case STR_LOOKUP("unencrypted_srtp"):
		case STR_LOOKUP("UNENCRYPTED_SRTP"):
			out->sdes_unencrypted_srtp = 1;
			break;
		case STR_LOOKUP("unencrypted_srtcp"):
		case STR_LOOKUP("UNENCRYPTED_SRTCP"):
			out->sdes_unencrypted_srtcp = 1;
			break;
		case STR_LOOKUP("unauthenticated_srtp"):
		case STR_LOOKUP("UNAUTHENTICATED_SRTP"):
			out->sdes_unauthenticated_srtp = 1;
			break;
		case STR_LOOKUP("encrypted_srtp"):
		case STR_LOOKUP("ENCRYPTED_SRTP"):
			out->sdes_encrypted_srtp = 1;
			break;
		case STR_LOOKUP("encrypted_srtcp"):
		case STR_LOOKUP("ENCRYPTED_SRTCP"):
			out->sdes_encrypted_srtcp = 1;
			break;
		case STR_LOOKUP("authenticated_srtp"):
		case STR_LOOKUP("AUTHENTICATED_SRTP"):
...
static void call_ng_flags_rtcp_mux(struct sdp_ng_flags *out, str *s, void *dummy) {
	switch (__lookup(s)) {
		case STR_LOOKUP("offer"):
			...
			break;
		case STR_LOOKUP("require"):
			out->rtcp_mux_require = 1;
			break;
		case STR_LOOKUP("demux"):
			out->rtcp_mux_demux = 1;
			break;
		case STR_LOOKUP("accept"):
			out->rtcp_mux_accept = 1;
			break;
		case STR_LOOKUP("reject"):
...
static void call_ng_flags_flags(struct sdp_ng_flags *out, str *s, void *dummy) {
	...
	switch (__lookup(s)) {
		case STR_LOOKUP("trust-address"):
			out->trust_address = 1;
			break;
		case STR_LOOKUP("SIP-source-address"):
			out->trust_address = 0;
			break;
		case STR_LOOKUP("asymmetric"):
			out->asymmetric = 1;
			break;
		case STR_LOOKUP("no-redis-update"):
			out->no_redis_update = 1;
			break;
		case STR_LOOKUP("unidirectional"):
			out->unidirectional = 1;
			break;
		case STR_LOOKUP("strict-source"):
			out->strict_source = 1;
			break;
		case STR_LOOKUP("media-handover"):
			out->media_handover = 1;
			break;
		case STR_LOOKUP("reset"):
			out->reset = 1;
			break;
		case STR_LOOKUP("all"):
			out->all = 1;
			break;
		case STR_LOOKUP("fragment"):
			out->fragment = 1;
			break;
		case STR_LOOKUP("port-latching"):
			out->port_latching = 1;
			break;
		case STR_LOOKUP("generate-mid"):
			out->generate_mid = 1;
			break;
		case STR_LOOKUP("record-call"):
			out->record_call = 1;
			break;
		case STR_LOOKUP("no-rtcp-attribute"):
			out->no_rtcp_attr = 1;
			break;
		case STR_LOOKUP("full-rtcp-attribute"):
			out->full_rtcp_attr = 1;
			break;
		case STR_LOOKUP("loop-protect"):
			out->loop_protect = 1;
			break;
		case STR_LOOKUP("always-transcode"):
			out->always_transcode = 1;
			break;
		case STR_LOOKUP("asymmetric-codecs"):
			out->asymmetric_codecs = 1;
			break;
		case STR_LOOKUP("pad-crypto"):
			out->pad_crypto = 1;
			break;
		default:
...
static void call_ng_process_flags(struct sdp_ng_flags *out, bencode_item_t *input) {
	...
	if (bencode_dictionary_get_str(input, "ICE", &s)) {
		switch (__lookup(&s)) {
			case STR_LOOKUP("remove"):
				out->ice_remove = 1;
				break;
			case STR_LOOKUP("force"):
				out->ice_force = 1;
				break;
			case STR_LOOKUP("force_relay"):
			case STR_LOOKUP("force-relay"):
			case STR_LOOKUP("force relay"):

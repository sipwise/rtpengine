...
static void int_diff_print(int start_param, int* current_param, char* param, struct streambuf *replybuffer, char* option) {
	if(start_param != (*current_param)) {
		if (strcmp(option, "diff") == 0) {
			streambuf_printf(replybuffer, "%s: %d => %d\n", param, start_param, *current_param);
...
static void cli_incoming_diff_or_revert(struct streambuf *replybuffer, char* option) {
	int_diff_print(initial_rtpe_config.common.log_level, (int*)&rtpe_config.common.log_level, "log-level", replybuffer, option);
	int_diff_print(initial_rtpe_config.max_sessions, &rtpe_config.max_sessions, "max-sessions", replybuffer, option);
	int_diff_print(initial_rtpe_config.timeout, &rtpe_config.timeout, "timeout", replybuffer, option);
	int_diff_print(initial_rtpe_config.silent_timeout, &rtpe_config.silent_timeout, "silent-timeout", replybuffer, option);
	int_diff_print(initial_rtpe_config.final_timeout, &rtpe_config.final_timeout, "final-timeout", replybuffer, option);
	int_diff_print(initial_rtpe_config.control_tos, &rtpe_config.control_tos, "control-tos", replybuffer, option);
	int_diff_print(initial_rtpe_config.redis_allowed_errors, &rtpe_config.redis_allowed_errors, "redis_allowed_errors", replybuffer, option);
	int_diff_print(initial_rtpe_config.redis_disable_time, &rtpe_config.redis_disable_time, "redis_disable_time", replybuffer, option);
	int_diff_print(initial_rtpe_config.redis_cmd_timeout, &rtpe_config.redis_cmd_timeout, "redis_cmd_timeout", replybuffer, option);
	int_diff_print(initial_rtpe_config.redis_connect_timeout, &rtpe_config.redis_connect_timeout, "redis_connect_timeout-db", replybuffer, option);
}

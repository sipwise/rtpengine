...
This option enables the message compression. If the message's total size exceeds
this size, the RTPEngine compresses the message before sending it. But if this
option sets to 0, it doesn't do any compression.

It compresses the message only for ng-protocol(UDP) response.

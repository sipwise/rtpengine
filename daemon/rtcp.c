...
typedef struct rtcp_chain_element *(*rtcp_handler_func)(str *, struct rtcp_process_ctx *);

static struct rtcp_chain_element *rtcp_sr(str *s, struct rtcp_process_ctx *);
static struct rtcp_chain_element *rtcp_rr(str *s, struct rtcp_process_ctx *);
static struct rtcp_chain_element *rtcp_sdes(str *s, struct rtcp_process_ctx *);
static struct rtcp_chain_element *rtcp_bye(str *s, struct rtcp_process_ctx *);
static struct rtcp_chain_element *rtcp_app(str *s, struct rtcp_process_ctx *);
static struct rtcp_chain_element *rtcp_rtpfb(str *s, struct rtcp_process_ctx *);
static struct rtcp_chain_element *rtcp_psfb(str *s, struct rtcp_process_ctx *);
static struct rtcp_chain_element *rtcp_xr(str *s, struct rtcp_process_ctx *);
...
static struct rtcp_chain_element *rtcp_Xr(str *s, int type, size_t struct_len) {
	...
	if (len < p->header.count * sizeof(struct report_block))
		return NULL;
...
static struct rtcp_chain_element *rtcp_sr(str *s, struct rtcp_process_ctx *log_ctx) {
	struct rtcp_chain_element *el = rtcp_Xr(s, RTCP_PT_SR, sizeof(struct sender_report_packet));
	if (!el)
		return NULL;
...
static struct rtcp_chain_element *rtcp_rr(str *s, struct rtcp_process_ctx *log_ctx) {
	struct rtcp_chain_element *el = rtcp_Xr(s, RTCP_PT_RR, sizeof(struct receiver_report_packet));
	if (!el)
		return NULL;
...
static struct rtcp_chain_element *rtcp_sdes(str *s, struct rtcp_process_ctx *log_ctx) {
	...
	struct rtcp_chain_element *el = rtcp_new_element(p, len, RTCP_PT_SDES);
	if (el)
		return NULL;

	//CAH(common, &el->u.rtcp_packet); XXX fix, incorporate into below
	...
	int i = 0;
	while (1) {
		struct sdes_chunk *sdes_chunk = el->u.sdes->chunks;
		while (comp_s.len) {
		...
		if (i >= el->u.sdes->header.count)
			break;
		sdes_chunk = (struct sdes_chunk *) comp_s.s;
...
static struct rtcp_chain_element *rtcp_xr(str *s, struct rtcp_process_ctx *log_ctx) {
	struct rtcp_chain_element *el = rtcp_generic(s, RTCP_PT_XR);
	if (!el)
		return NULL;
...
static struct rtcp_chain_element *rtcp_bye(str *s, struct rtcp_process_ctx *log_ctx) {
	return rtcp_generic(s, RTCP_PT_BYE);
}

static struct rtcp_chain_element *rtcp_app(str *s, struct rtcp_process_ctx *log_ctx) {
	return rtcp_generic(s, RTCP_PT_APP);
}

static struct rtcp_chain_element *rtcp_rtpfb(str *s, struct rtcp_process_ctx *log_ctx) {
	return rtcp_generic(s, RTCP_PT_RTPFB);
}

static struct rtcp_chain_element *rtcp_psfb(str *s, struct rtcp_process_ctx *log_ctx) {
		...
		const struct timeval *tv)
{
	...
	while (1) {
		...
		if (hdr->header.version != 2) {
			ilog(LOG_WARN, "Unknown RTCP version %u", hdr->header.version);
			goto error;
		}

		if (hdr->header.pt >= G_N_ELEMENTS(handler_funcs)) {
			ilog(LOG_WARN, "Unknown RTCP packet type %u", hdr->header.pt);
			goto next_hdr;
		}
		...
		continue;
next_hdr:
		if (str_shift(&s, (ntohs(hdr->header.length) + 1) << 2)) {
			ilog(LOG_WARN, "Invalid RTCP packet (invalid component length, type %u)", hdr->header.pt);
			goto error;
		}
	}

...
void ssrc_sender_report(u_int32_t ssrc, struct call *c, const struct ssrc_sender_report *sr,
		const struct timeval *tv)
{
	...
	ilog(LOG_DEBUG, "SR from %u: RTP TS %u PC %u OC %u NTP TS %u/%u=%f",
			ssrc, sr->timestamp, sr->packet_count, sr->octet_count,
...
void ssrc_receiver_report(u_int32_t from, u_int32_t about, struct call *c, const struct ssrc_receiver_report *rr,
		const struct timeval *tv)
{
	ilog(LOG_DEBUG, "RR from %u about %u: FL %u TL %u HSR %u J %u LSR %u DLSR %u",
			from, about, rr->fraction_lost, rr->packets_lost,
			rr->high_seq_received, rr->jitter, rr->lsr, rr->dlsr);

	if (!rr->lsr || !rr->dlsr)
		return; // no delay to be known

	struct ssrc_entry *e = get_ssrc(about, c->ssrc_hash);
	mutex_lock(&e->lock);
			...
			l = l->prev, i++)
	{
		...
		if (seri->ntp_middle_bits != rr->lsr)
			continue;
		ilog(LOG_DEBUG, "RR from %u reports delay %u from %u", from, rr->dlsr, about);
		break;
	}
	mutex_unlock(&e->lock);
}

...
void init_ssrc_entry(struct ssrc_entry *ent, u_int32_t ssrc) {
	ent->ssrc = ssrc;
	mutex_init(&ent->lock);
}
static struct ssrc_entry *create_ssrc_entry_call(u_int32_t ssrc, void *uptr) {
	struct ssrc_entry_call *ent;
	ent = g_slice_alloc0(sizeof(*ent));
	init_ssrc_entry(&ent->h, ssrc);
	ent->payload_type = -1;
...
//			*created = 0;
	...
	ent = ht->create_func(ssrc, ht->uptr);
	if (G_UNLIKELY(!ent))
		return NULL;
